...
static func _ensure_materials() -> void:
	if not material_cache.is_empty():
		return
	material_cache["floor"] = _make_material(TEXTURE_DARK_PATH)
	material_cache["goal"] = _make_material(TEXTURE_GREEN_PATH)
	material_cache["weak"] = _make_material(TEXTURE_ORANGE_PATH)
	material_cache["switch"] = _make_material(TEXTURE_RED_PATH)
	material_cache["button"] = _make_material(TEXTURE_PURPLE_PATH)
	material_cache["teleporter"] = _make_material(TEXTURE_BLUE_PATH)
	material_cache["bridge_active"] = _make_material(TEXTURE_LIGHT_PATH)
	material_cache["bridge_inactive"] = _make_material(TEXTURE_DARK_PATH, Color(0.5, 0.5, 0.5))
